...
class DiscordBot {
    constructor(config) {
        ...
        this.serverLimitBreakRolesByCommand = {};
        ...
        // Limiters
        this.anagramLimiter = new RateLimiter(10, 'minute');
    ...
    // youtubeObject: {link: "", channelID: "", userID: ""}
    playYoutube(youtubeObject, callback) {
        ...
        const channelID = youtubeObject.channelID;
        const userID = youtubeObject.userID;
        ...
        this.ytStream.on('info', info => {
            // The duration given back by youtube-dl is retarded
            const secondsInUnit = [1, 60, 3600, 86400];
            const matches = info.duration.match(/(\d{1,2})+/g).reverse();
            const duration = matches.reduce((prev, current, i) => { return prev + (current * secondsInUnit[i]); }, 0);

            if (duration > 600 && !this.roles.userCanLimitBreak(this, "youtube", userID, channelID)) {
